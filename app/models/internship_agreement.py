...
class InternshipAgreementBase(SQLModel):
    ...
    # and is_active is checked by some endpoints (entry creation and deletion) to allow students to modify data
...
class InternshipAgreementInDB(InternshipAgreementBase, table=True):
    ...
    # special SQLAlchemy attribute (declarative class attribute) to define table advanced configurations 
    # accepts positional & keyword arguments specified either as Dictionary (table options) or as Tuple (constraints)
    __table_args__ = (
        UniqueConstraint("student_id", "company_id", name="unique_student_company"),  # => the comma defines the value as a TUPLE
    )
