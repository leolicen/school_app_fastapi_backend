...
class AuthService():
    ...
    def create_access_token(id: uuid.UUID, expires_delta: timedelta | None = None) -> str:
        """Create access token with student id and expiration value."""
        ...
        jti = str(uuid.uuid4())
        # payload (1/3 JWT elements together with header and signature): "sub" claim (subject) + "exp" claim + "jti" claim
        payload = {
            "sub": str(id),
            "exp": expire,
            "jti": jti  # JWT ID
    ...
    async def validate_access_token(self, token: str) -> AccessTokenData:
        """Decode access token and return student id as AccessTokenData."""
        try:
    ...
    def create_reset_token(email: EmailStr, session: Session) -> str:
        ...
        # student_service.get_student_by_email() is not used to avoid circular import errors
        student_in_db = session.exec(
            select(StudentInDB).where(StudentInDB.email == normalized_email)
        ).first()

        # if not, log internally and exit
    ...
    def create_refresh_token(student_id: uuid.UUID, session: Session) -> str:
        """Create and persist a new refresh token for the given student. Returns the raw (unhashed) token."""
        # create raw token
